...
operational tooling, and a host-neutral adapter. We give an inductive
...
\CompatibilitySummary. Two clean package builds produced byte-identical artifacts; Twine,
isolated base/server/client imports and CLI startup on Python~3.14, Bandit~1.9.4 at medium/high
severity, and pip-audit~2.10.1 passed. These are bounded tool results, not an external security
...
transaction per stage; verify-only has no write. The source-frozen worktree was not yet committed,
and the single-host in-process measurements exclude HTTP, network, container scheduling, external
authority/audit services, and independent failure domains. Its SQLite~3.50.4 runtime is not
production-admissible under the release gate; these are development-path measurements bound to the
same source digest as the acceptance evidence. The reviewed machine-readable baseline is retained
under \code{benchmarks/baselines}.
...
single-writer database; latency includes lock wait, and its 64.837-ms p95 shows why the result must
...
digest \ProductionEvidenceDigestDisplay. The runner recorded an uncommitted but source-frozen
\ProductionSourceCount-file snapshot with digest \ProductionSourceDigestDisplay; that runtime tree
was subsequently captured in release commit \ProductionSourceCommit, whose SSH signature was
verified locally. All three wardens ran SQLite~\ProductionSQLiteVersion{} and agreed on signed
manifest digest \ProductionManifestDigestDisplay.
...
$image_name = "127.0.0.1:25000/astraldeep/lets"
$image_digest = "sha256:" + `
  "2f002bc691685930e07bf5d4e297f2b9ba13617aad222d512afc7b3d59a8e8ad"
