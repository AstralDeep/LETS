...
branch revocation, canonical signed JSON, an authenticated HTTP API and a strict-decoding,
bounded-retry client, operational tooling, and a host-neutral adapter. We give an inductive
...
collected unit, integration, security, property, recovery, and end-to-end cases. A clean-volume
Docker acceptance run used three distinct processes, databases, and Ed25519 keys; it exercised a
bidirectional partition, transfer reordering and replay, abrupt restart, independent executor
verification, and aggregate conservation. The runtime is production-intent pre-1.0 software, not
a claim of Byzantine tolerance, arbitrary live migration, or generic exactly-once physical effects.
\end{abstract}
...
This artifact makes five concrete contributions.

\begin{enumerate}
  \item \impl{} A real multi-node runtime in which each warden has an independent process,
  database, signing key, replay store, and initial escrow share.
  ...
  \item \checked{} Reproducible unit, security, property, crash-recovery, bounded-model, and real
  three-node fault evidence, with the evidence types kept distinct.
\end{enumerate}
...
Partition behavior & Local transitions at A and B while both directed peer links are disabled & \measured \\
Restart behavior & Target killed with \code{SIGKILL}, restarted on its persisted volume, replay still denied & \measured \\
...
evaluators, database, clock within the configured uncertainty, signing key, manifest trust roots,
protected executors, and the host isolation that prevents subjects from writing those components.
...
\draw[arr] (a) -- node[right,font=\scriptsize]{receipt} (ea);
\draw[arr] (b) -- node[right,font=\scriptsize]{receipt} (eb);
\draw[arr] (c) -- node[right,font=\scriptsize]{receipt} (ec);
...
\end{tikzpicture}
\caption{Reference topology. The Compose acceptance deployment instantiates three independent
warden processes, stores, and keys; its test exercises one executor with a replay store outside the
warden database. Protected domains may deploy similarly isolated executors per site. Ephemeral
subjects call wardens but are not escrow replicas.}
\label{fig:topology}
\end{figure}
...
registered warden/key pair that is currently valid at verification time. When local
\code{ExecutorPolicy} fields or allowlists are configured, it additionally pins tenant, envelope,
epoch, policy digest, machine digest, and warden. Its durable claim then requires the lease
sequence to advance and claims both receipt identifier and nonce. This supplies at-most-once authorization consumption. A generic
receipt cannot make an arbitrary physical effect exactly once: the executor must bind receipt
claim and effect in one domain transaction, make the effect idempotent, or use compensation.
...
\draw[arr] (s) -- (p);
\draw[arr] (p.east) -- node[above,font=\scriptsize]{voucher $(seq,digest,amount)$} (a.west);
\draw[arr] (a.west) -- node[below,font=\scriptsize]{signed acknowledgement} (f.east);
\draw[arr] (f) -- (c);
\draw[arr,dashed] (c.west) -- node[below,font=\scriptsize]{signed watermark} (f.east);
\end{tikzpicture}
...
\addlinespace
Cloned source database and key run concurrently & Outside version \ArtifactVersion{} model; can duplicate authority & Unsafe/non-goal \\
\bottomrule
\end{tabularx}
\end{table}
...
the initial warden/key identifier and SHA-256 public-key fingerprint; signer substitution therefore
fails startup. Manifest-backed service startup reloads strict JSON and re-verifies the manifest
...
bundled static bearer authenticator is bootstrap tooling and stores only token digests. Production
deployments should inject an OIDC, mTLS, gateway, or workload-identity verifier.
...
an Ed25519 signature. A dedicated SQLite replay store atomically prunes expired entries and claims
new nonces. Peer signatures authenticate content but do not provide confidentiality or hide
metadata; TLS remains mandatory outside explicit development topology.

The operator surface provides liveness, clock-aware readiness, invariant snapshots, audit
verification, bounded audit pagination, and metrics for pools, consumption, transfer
watermarks/gaps, and outbox lag. A verified SQLite backup command refuses an existing destination
and validates the produced snapshot. Restore is operationally safe only after fencing the original
node.
...
\code{storage.sqlite} & Versioned schema, durability, repositories, and transactional conservation checks \\
...
\code{executor} & Independent receipt verification and durable at-most-once claim \\
...
The theorem excludes a compromised warden signing nonexistent vouchers, a simultaneously running
database/key clone, manifest trust-root compromise, and direct database modification.
...
claims the receipt. A missing, invalid, stale, or replayed receipt fails closed. \hfill$\square$
...
Docker acceptance & \DockerScenarioResult & One composite clean-volume fault scenario on one Windows/Docker host; not a throughput or multi-host WAN study. \\
...
The release gate ran the non-e2e suite with \code{ResourceWarning} promoted to an error and
branch-aware coverage enabled: \CoreTestSummary, against a configured \CoverageFloor{} floor. The
real Docker e2e case is reported separately below so an unavailable Docker daemon cannot be hidden
inside the core-suite result. A separate exact all-extras environment on Python~3.11.15 produced
\CompatibilitySummary. The package matrix built and inspected the wheel and source distribution,
smoke-tested base/client/server imports on Python~3.14, and completed Twine, wheel-content, Bandit,
and pip-audit checks without a finding; these are bounded tool results, not an external security
certification. The wheel was \WheelBytes{} bytes with digest \WheelDigestDisplay; the source
distribution was \SdistBytes{} bytes with digest \SdistDigestDisplay.

\subsection{Post-optimization storage measurements}
...
transaction per stage; verify-only has no write. The worktree was uncommitted, and the single-host
in-process measurements exclude HTTP, network, container scheduling, and independent failure
domains. The reviewed machine-readable baseline is retained under \code{benchmarks/baselines}.

\begin{table}[ht]
\centering
\caption{Post-optimization latency and wall-time rate. The 100-sample p95 values are indicative,
...
single-writer database; latency includes lock wait, and its 80.881-ms p95 shows why the result must
not be generalized to arbitrary write concurrency.
...
\measured{} The isolated diagnostic median grew from \FKDiagnosticSmall{} ms at 10 receipt rows to
\FKDiagnosticLarge{} ms at 1,000 rows (\FKDiagnosticGrowth). A paired 1,000-row authorize result
fell 17.8\%, but security hardening and the cryptographic-backend change occurred between runs, so
the paper does not attribute that full delta to the foreign-key optimization.
...
legacy row-scan medians were \SnapshotLegacy; aggregate medians were \SnapshotAggregate. The
1,000-row observation was \SnapshotLargeReduction{} lower and the aggregate medians were flat over
this bounded range. The legacy observations were taken immediately before the edit on the same host
but predate the reusable harness.

A read-only keeper-connection experiment was rejected: four order-alternated trials gave
\KeeperBaseline{} without the keeper and \KeeperExperiment{} with it, a \KeeperPenalty{} penalty.
...
graph contains \LockedPackages{} packages in a \LockBytes-byte lockfile; the PyNaCl/libsodium
backend and its CFFI support occupy \InstalledCryptoBytes{} installed bytes in that environment.
The lock contains no \code{cryptography} package.

\subsection{Real three-node fault acceptance}

The clean-volume Compose run used Python \DockerPythonVersion{}, Docker Engine
\DockerEngineVersion{}, and Docker Compose \DockerComposeVersion{}. It ran three warden containers
with distinct process IDs, independent SQLite volumes, and three distinct Ed25519 keys from one
operator-signed manifest. Initial shares $150+100+50$ exactly partitioned a
\DockerInitialBudget-unit envelope.

The acceptance runner rejected any source change across the run. Its explicit, non-circular
\DockerRuntimeInputCount-file runtime/acceptance input set had digest
\DockerRuntimeInputDigestDisplay; the complete \DockerSourceCount-file Git-visible snapshot
had digest \DockerSourceDigestDisplay. The rebuilt runtime image content identifier was
\DockerRuntimeImageDigestDisplay. Paper prose and derived evidence are excluded from the
runtime-input set, so this manuscript rebuild does not invalidate the recorded execution.

The scenario delivered transfer sequence 2 before sequence 1. The target held its contiguous
watermark at 0 and returned stable acknowledgements for duplicate application delivery. The harness
disabled both directed A--B proxy links: peer delivery failed while its two vouchers remained
durable and A and B each committed a local authorization receipt. It then killed warden B with
\code{SIGKILL}; the restarted process had a new PID and the same persisted key. An identical signed
HTTP request was rejected with status 409 and code \code{replay_detected} both before and after
restart. After link restoration, the production dispatcher automatically accepted and finalized both transfers,
delivered the bilateral checkpoint, advanced both compacted high-water marks to 2, and converged to
zero pending delivery records and zero prepared transfers.

An executor outside the warden database verified one receipt, claimed it in its own SQLite replay
store, reopened that store, and rejected the same receipt. At the final consistent cut, cumulative
inbound and outbound transfers were both \DockerTransferred, the sum of free pool, lease residual,
and consumed rights across nodes was \DockerInitialBudget, every local invariant reported healthy,
and all three signed audit chains verified. The pytest result was \DockerScenarioResult; the full
provenance-bound runner took \DockerEvidenceDuration. The manifest digest and machine-readable
scenario are retained in the artifact evidence record; the run's manifest digest was
\DockerManifestDigestDisplay.

This scenario establishes that the hard acceptance path is not an in-process simulation. It does
not establish geographic independence, behavior under arbitrary schedules, long-duration
stability, or Byzantine tolerance.
...
\begin{table}[ht]
\centering
\caption{Standards and draft composition. ``Profile'' means a mapping boundary, not bundled conformance.}
\label{tab:standards}
\small
...
behavior for parent debit, command idempotency, and executor claim.

Hash-chained signed audit makes row modification or deletion detectable when the complete chain
and trusted key are available. It does not provide an external anti-rollback anchor: restoring an
older database and key together can present an internally valid old prefix. Operators should
export and anchor checkpoints outside the node when rollback detection matters.

\subsection{Residual risks and explicit boundaries}

\begin{table}[ht]
\centering
\caption{Version \ArtifactVersion{} boundaries.}
\label{tab:limits}
\small
...
Restore clones & Never run original and restored copies of one database/key concurrently. Fence the original before restore. \\
...
Clock rollback & Durable time floors block ordinary rollback but not a cloned or operator-rewritten database. Large forward jumps can expire authority early. \\
...
otherwise-unexpired receipts and undelivered transfer, revocation, or checkpoint records. Active
version~1 online warden keys should therefore normally omit finite \code{not_after}. If one is
...
bootstrap machinery; production systems should prefer an external or hardware-backed key provider.
...
budget dimension and binds all initial keys, endpoints, policies, and epoch metadata. Each private
key, database, replay database, configuration, and manifest trust root form one authority unit.
Production endpoints use TLS; the cleartext Compose topology and public default token are fault-test
fixtures only.

Operators should monitor readiness, disk and WAL growth, write latency and busy errors, clock
uncertainty, lease and receipt expiry, transfer gaps and watermarks, outbox lag, invariant status,
and audit verification. A source voucher is never manually refunded after timeout. Backups are
offline or coordinated snapshots; restores are reconciled against peer watermarks and audit
checkpoints before returning to readiness. Idempotency and signed audit history are not deleted by
hand.
...
but there is no machine-checked refinement connecting every SQL transaction and crash point to
Eq.~\ref{eq:global}. The bounded checker covers only one small finite state space. Independent
audit and multi-language canonicalization interoperability are still required.

The reported Docker run is one composite scenario on one host. It verifies independent operating
system processes and storage volumes, but not independent failure domains, heterogeneous
architectures, real WAN clocks, sustained partitions, rolling upgrades, or long-duration load.
...
configuration-epoch rollover, externally anchored audit/checkpoints, a validated PostgreSQL
adapter, hardware-backed keys, policy analysis, and workload-informed rights placement. Active
...
and partition without double credit under the trusted-warden model.
...
boundaries around Byzantine faults, restores, migration, external effects, and policy correctness.
...
The post-optimization benchmark suite uses the repository-local Python environment:
\begin{verbatim}
...
The Docker acceptance runner validates the exact five project-owned volumes it may recreate,
starts from clean state, builds the runtime, waits for all health checks and directed fault
proxies, writes machine-readable evidence, and stops containers in a \code{finally} path. The
test-only volumes remain for operator inspection.
...
\begin{enumerate}
  ...
  \item the receipt identifier and nonce are claimed once in a filesystem-backed replay store.
\end{enumerate}
